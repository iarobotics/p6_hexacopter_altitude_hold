...
void loop(){
  ...
  if(Serial.available() > 0){
    ...
    //Motor assignment according to ardupilot hexa
    ...
    if(data == '1')Serial.println("Test motor 1 (right CW.)");
    if(data == '2')Serial.println("Test motor 2 (left CCW.)");
    if(data == '3')Serial.println("Test motor 3 (front left CW.)");
    if(data == '4')Serial.println("Test motor 4 (rear right CCW.)");
    if(data == '5')Serial.println("Test motor 5 (front right CCW)");
    if(data == '6')Serial.println("Test motor 6 (rear left CW)");
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////
  if(data == '1' || data == '2' || data == '3' || data == '4' || data == '5' || data == '6' || data == '7'){          //If motor 1, 2, 3 or 4 is selected by the user.
    ...
    if(new_function_request == false){                                                  //When the throttle was in the lowest position do this.
      ...
      if(data == '5' || data == '7')esc_5 = receiver_input_channel_3;
      else esc_5 = 1000;                                                   
      if(data == '6' || data == '7')esc_6 = receiver_input_channel_3;
      else esc_6 = 1000;                                                   
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      ///We don't want the esc's to be beeping annoyingly. So let's give them a 1000us pulse while calibrating the gyro.
        ...
        delayMicroseconds(1000);                                                        //Wait 1000us.
        PORTD &= B10010111;                                                             //Set digital port 3, 5, 6 low
        PORTB &= B11110001;                                                             //Set digital port 9, 10, 11 low                                                         //Set digital poort 4, 5, 6 and 7 low.
...
void wait_for_receiver(){
  ...
  timer = millis() + 10000;////////
  while(timer > millis() && zero < 15){                                                            //Stay in this loop until the 4 lowest bits are set
...
void esc_pulse_output(){
  ...
  timer_channel_5 = esc_5 + zero_timer;
  timer_channel_6 = esc_6 + zero_timer;
  a = true;

  //while(PORTD > 0){                                            //Execute the loop until digital port 3 to 11 is low.
  //  esc_loop_timer = micros();                                   //Check the current time.
  //  if(timer_channel_1 <= esc_loop_timer)PORTD &= B11110111;     //When the delay time is expired, digital port 3 is set low.
  ...
  //  if(timer_channel_4 <= esc_loop_timer2)PORTB &= B11111101;     //When the delay time is expired, digital port 9 is set low.
  //  if(timer_channel_5 <= esc_loop_timer2)PORTB &= B11111011;     //When the delay time is expired, digital port 10 is set low.
  //  if(timer_channel_6 <= esc_loop_timer2)PORTB &= B11110111;     //When the delay time is expired, digital port 11 is set low.
  //}

  while(a==true){                                                //Execute the loop until digital port 3 to 11 is low.
    if((PORTD == 0)&&(PORTB == 0))a=false;
    esc_loop_timer = micros();                                   //Check the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTD &= B11110111;     //When the delay time is expired, digital port 3 is set low.
    if(timer_channel_2 <= esc_loop_timer)PORTD &= B11011111;     //When the delay time is expired, digital port 5 is set low.
    if(timer_channel_3 <= esc_loop_timer)PORTD &= B10111111;     //When the delay time is expired, digital port 6 is set low.

    if(timer_channel_4 <= esc_loop_timer)PORTB &= B11111101;     //When the delay time is expired, digital port 9 is set low.
    if(timer_channel_5 <= esc_loop_timer)PORTB &= B11111011;     //When the delay time is expired, digital port 10 is set low.
    if(timer_channel_6 <= esc_loop_timer)PORTB &= B11110111;     //When the delay time is expired, digital port 11 is set low.
  }
